...
- **Bot-mode Telethon singleton** (`core/telegram.py`) — `TelegramClient(StringSession(), api_id, api_hash, receive_updates=False)` + `client.start(bot_token=...)`. Token в `start()`, НЕ в конструктор. In-memory `StringSession` (НЕ file session) — bot-token auth мгновенная, file persistence не нужен; раньше file session вызывала SQLite lock conflicts (tests + container одновременно). Поле `Settings.session_path` сохранено для backwards compat, но не используется
