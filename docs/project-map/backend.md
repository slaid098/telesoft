...
key_files:
  ...
  - src/telesoft/core/runner.py — JobRunner (asyncio.Semaphore, cooperative cancel, status transitions)
  ...
  - src/telesoft/api/routers/jobs.py — jobs router (POST replace-link, GET /api/jobs, GET /api/jobs/{id}, GET /api/jobs/{id}/logs, POST /api/jobs/{id}/cancel, auth on whole router, no prefix)
  ...
  - src/telesoft/schemas/job.py — Pydantic models (ReplaceLinkRequest/JobResponse/JobListResponse/LogResponse/LogListResponse/WsEvent) + now_iso() helper
...
│   └── runner.py          # JobRunner: asyncio.Semaphore(max_concurrency), submit(job_id, chat_id, message_ids, pattern, new_link) → asyncio.create_task, cancel(job_id) cooperative (_cancelled set + task.cancel), worker _run_job (mark running → publish job_started → set total → loop message_ids: check _cancelled → replace_link_in_post → write log → publish progress → mark done/cancelled/failed + publish event)
...
│       ├── jobs.py        # APIRouter(tags=["jobs"], dependencies=[Depends(require_auth)]) БЕЗ prefix: POST /api/channels/{channel_id}/replace-link (201, validates channel/pattern/post_urls/chat_id matching), GET /api/jobs (filters + pagination), GET /api/jobs/{id} (404), GET /api/jobs/{id}/logs (404 + pagination), POST /api/jobs/{id}/cancel (404/409 if terminal)
...
└── schemas/      # Pydantic request/response models
    ...
    └── job.py       # ReplaceLinkRequest(post_urls, pattern, new_link), JobResponse (+ from_row), JobListResponse, LogResponse (+ from_row), LogListResponse, WsEvent (+ from_event, optional fields for all event types), now_iso() helper
```
...
- **JobRunner** (`core/runner.py`) — `asyncio.Semaphore(max_concurrency)` для лимита concurrent edits (Telegram rate limits). `submit(job_id, chat_id, message_ids, pattern, new_link)` → `asyncio.create_task`, registry `_tasks[job_id] = task`. Все параметры явно (НЕ из БД — telesoft `edit_jobs` не имеет JSON-колонок, отличие от media-gen). Worker: `async with sem` → mark running + publish `job_started` → set total → loop message_ids: check `_cancelled` → `replace_link_in_post` → update progress → write log → publish `progress`. В конце: `done` + `completed` event. CancelledError → `cancelled` event + re-raise. Exception → `failed` event. `finally` — `_tasks.pop` + `_cancelled.discard`
...
- **Jobs router БЕЗ prefix** (`api/routers/jobs.py`) — `APIRouter(tags=["jobs"], dependencies=[Depends(require_auth)])` без prefix, т.к. эндпоинт `/api/channels/{channel_id}/replace-link` не подходит под `/api/jobs`. Полный путь в `@router.post(...)`. Auth на весь роутер (PR#20 паттерн)
- **`_resolve_chat_and_ids`** — сверяет parsed URL chat_id/username с `channel.telegram_id`/`channel.username`. `isinstance(identifier, int)` → private (сравнение с `telegram_id`), иначе public (сравнение с `username`). 422 при mismatch. `chat_id` для runner = `channel.telegram_id` всегда (int надёжнее — не требует resolve_entity)
