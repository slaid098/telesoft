...
key_files:
  - tests/conftest.py — fixtures: mock_settings, mock_db, create_channel/create_job factories, mock_message, mock_telethon_client, mock_telethon_get_message, mock_telethon_edit_message, mock_runner, authed_client; mock_channel_messages helper + MockMessage.date поле (PR#32)
  ...
  - tests/test_telegram_client.py — bot-mode Telethon client wrapper (19 тестов: singleton/idempotent/by-ID fetch/batch/edit/resolve/bot-info/stop + 9 get_last_messages: success/empty/limit>max_id/binary_search/all_empty/filters_empty/flood_retry/flood_max_retries/delay_between_requests)
...
- **`monkeypatch.setattr(telegram_module.asyncio, "sleep", AsyncMock())`** (PR#32) — patch'ит `asyncio.sleep` в модуле `core.telegram` для тестов delay/retry (НЕ глобальный asyncio.sleep). Позволяет assert'ить `await_args_list` (количество вызовов, args). `test_delay_between_requests` проверяет `c.args[0] == settings.telegram_request_delay`, `test_flood_wait_retry` — `c.args[0] == 3` (exc.seconds + 1 = 2 + 1)
...
- **Coverage 93.95%** после PR #32 (gate 80% пройден). Uncovered: `core/runner.py` 80% (defensive `assert sem is not None`, `is_running`/`get_task`/`active_count`, exception path в `_mark_final` без error column), `api/routers/jobs.py` 95% (defensive `assert row is not None` после `_get_job_or_404`, username match branch), `schemas/job.py` 94% (`WsEvent.from_event` — tested via integration, не unit). 110 тестов (было 101 — +9 get_last_messages PR#32)
