...
key_files:
  ...
  - web/src/routes/+layout.svelte — app shell (top nav bar Главная/Каналы/Задачи PR#74, header, mobile bottom nav grid-cols-3 PR#74)
...
└── src/
    ...
    ├── routes/
    │   ├── +layout.svelte  # app shell PR#74: top nav bar (wordmark «telesoft» кликабельный → /, horizontal nav Главная/Каналы/Задачи на desktop sm+, «Вы вошли как» + «Выйти» справа), mobile bottom nav (grid-cols-3 PR#74, 3 пункта), isActive(item) хелпер со строгим сравнением для "/" (иначе startsWith("/") всегда true); login guard {#if isLogin} сохранён; Svelte 5 runes
...
- **Bottom nav `grid-cols-3`** (PR#38 → PR#74) — mobile bottom nav в `+layout.svelte`: `grid grid-cols-3` (PR#74: было `grid-cols-2` для 2 пунктов, стало 3 пункта Главная/Каналы/Задачи). Иконки `text-xl`, лейблы `text-xs`, `py-3` (≥44px touch target). Sidebar удалён в PR#74 — заменён верхним навбаром в `<header>` (wordmark + horizontal nav на desktop sm+).
