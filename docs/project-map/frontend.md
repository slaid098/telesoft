...
key_files:
  ...
  - web/src/lib/types.ts — TS types mirroring backend Pydantic (Channel/Job/Log/WsEvent/WsEventType/WsEventPayload/ReplaceLinkRequest; PR#58: ReplaceMode, PreviewRequest/Item/Response, PatternResponse/ListResponse/CreateRequest, ReplaceLinkRequest+mode+keep_tail; PR#62: ReplaceLinkRequest+post_link, PreviewRequest+post_link)
  ...
  - web/src/lib/components/ReplaceLinkForm.svelte — replace-link form (PR#58 редизайн: 3 таба Simple/Library/Advanced via role="tablist", keep_tail checkbox, Preview+Run buttons, runSignal prop; PR#36 limit 1..1000; PR#38 touch target py-2.5; PR#62: +post_link field "Ссылка на последний пост" обязательное, placeholder `https://t.me/channel/140`, canSubmit +trimmedPostLink.length>0)
...
└── src/
    ...
    │   ├── types.ts      # TS types mirroring backend Pydantic: Channel, Job, Log, WsEvent, WsEventPayload, JobStatus, ReplaceLinkRequest={pattern, new_link, post_link, limit, mode, keep_tail} (PR#58: +ReplaceMode, PreviewRequest/Item/Response, PatternResponse/ListResponse/CreateRequest; PR#62: +post_link в ReplaceLinkRequest + PreviewRequest)
    │   └── components/
...
- **TS types mirroring backend Pydantic** — `Channel`/`Job`/`Log`/`WsEvent`/`JobStatus`/`ReplaceLinkRequest` (PR#20/22/36). `ReplaceLinkRequest={pattern, new_link, post_link, limit, mode, keep_tail}` (PR#62 — `post_link` добавлен, mirrors backend `src/telesoft/schemas/job.py` PR#62; PR#34 — `post_urls` убран, mirrors backend PR#34). `JobStatus="done"` (не "completed"), `WsEvent` flat structure (не `{type, data}`), `MeResponse={user: string}` (не `{username}`)
