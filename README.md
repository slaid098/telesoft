...
Редактор постов Telegram-каналов — замена ссылок через бота-админа.

## Описание

telesoft — веб-приложение для администраторов Telegram-каналов. Позволяет массово заменять ссылки в постах канала за один запуск: пользователь входит в админ-панель, выбирает канал, настраивает шаблон замены и запускает задачу. Бот-админ редактирует посты в фоне, прогресс транслируется в UI в реальном времени через WebSocket.
...
- **Каналы**: список, добавление, удаление (CASCADE — удаляет связанные задачи и логи), детальная страница с формой замены
